...
PATH = config.path_to_shalla_list

'''
Outputs all the urls/domains corresponding to a certain group. The array outputed consists of a list of tuples with a url and its group name. 
'''
...
'''
Finds all the names of the groups in the database. The group names are the names of the folders. Returns a dictionary of group names and their corresponding index values, and a single list of (url/domain, group_name)s. 
'''
...
'''
Uses the function collect to create a database for group_name and group_index, and another database for url, group_index and eventually sha1, cert_time, timestamp, and error. 
'''


def create_blacklist_veritawall():
    try:
        ...
        cur.execute(f'''CREATE TABLE {config.main_table}
            ...
            TIMESTAMP         TIMESTAMP,
            ERROR       VARCHAR(100));''')
        print("Blacklist_Veritawall Table created successfully")
    ...
    try:
        ...
        for key, value in group_dict.items():
            ...
            cur.execute(
                f"INSERT INTO {config.groupe_table} (ID,GROUPE) VALUES ({value},'{key}');")
        con.commit()
        con.close()
    except:
    ...
    try:
        ...
        for i, f in enumerate(file_out):
            try:
                ...
                if "'" not in val:
                    cur.execute(
                        f"INSERT INTO {config.main_table} (URL,GROUPE) VALUES ('{val}',{group_dict[f[1]]});")
