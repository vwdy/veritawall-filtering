...
def create_blacklist_veritawall():
    try:
        ...
        try:
            cur.execute(
                f"CREATE INDEX b_tree_idx ON {config.main_table} USING btree (url)")
    ...
    try:
        ...
        for i, line in enumerate(content):
            ...
            try:
                cur.execute(
                    f"INSERT INTO {config.main_table} (ID,URL,GROUPE) VALUES ({i}, '{value[0]}', {group_dict[value[1]]});")
            except Exception as e:
                print(e)
                pass
        con.commit()
...
def fill_suricata_database():
    try:
        ...
        print(todays_date)
        cur.execute(
            f"SELECT DISTINCT sha1,groupe FROM (SELECT sha1,groupe FROM {config.main_table} WHERE sha1 is not null AND certtime > TO_DATE('{todays_date}','YYYYMMDD')) AS t;")
        data = cur.fetchall()
        for i, line in enumerate(data):
            sha1 = line[0]
            groupe = line[1]
            cur.execute(
                f"INSERT INTO {config.suricata_table} (ID,GROUPE,SHA1) VALUES ({i}, {groupe}, '{sha1}');")
        con.commit()
        con.close()
    except Exception as e:
        print(e)
